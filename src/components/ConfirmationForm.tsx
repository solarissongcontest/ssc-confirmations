  ...
  const options = [
    ...
    ...(allowImmediate
      ? [
          {
            value: "immediately",
            label: "Can be revealed immediately",
          },
        ]
      : []),
    {
...
export interface ConfirmationFormProps {
  ...
  /** Secure edit link token for one specific submission. */
  editToken?: string;
  ...
  /** Live availability reason from the parent page. */
...
export function ConfirmationForm({
  ...
  const persistDraft =
    useServerFn(saveDraft);

  const fetchDraft =
    useServerFn(loadDraft);

  const findMine =
    useServerFn(findMySubmission);

  const checkAvailability =
    useServerFn(getRoundAvailability);
  ...
  const [done, setDone] = useState<
    null | "submitted" | "not_participating"
  >(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // Local draft is still available.
      }
  ...
  function validate(
    ...
    if (current === 0) {
      ...
      if (
        ...
        e.country_account =
          "Add the delegation account, or answer No above.";
      }
    }
    ...
    if (
      ...
      e.selection_method =
        "Please choose how you will select your entry.";
    }
    ...
    if (
      ...
      if (!data.song_url.trim()) {
        e.song_url =
          "Song link is required.";
      } else if (
        !isValidUrl(
          data.song_url,
        )
      ...
      if (
        ...
        e.preview_start =
          "Enter a start time as MM:SS (e.g. 01:12).";
      }
      ...
      if (
        ...
        if (
          !data.replacement_video_url.trim()
        ) {
          e.replacement_video_url =
            "A replacement video link is required.";
        } else if (
          !isValidUrl(
    ...
    if (
      ...
      if (
        ...
        if (
          ...
          e.nf_entries =
            "Add at least one entry, or tick “NF entries not known yet”.";
        }
      ...
      if (
        data.nf_result_date_type ===
          "approximate" &&
        !data.nf_result_approximate_text.trim()
      ) {
        e.nf_result_date_type =
          "Describe the approximate date.";
      }
    ...
    if (current === 4) {
      ...
      if (
        data.reveal_date_type ===
          "approximate" &&
        !data.reveal_approximate_text.trim()
      ) {
        e.reveal_date_type =
          "Describe the approximate date.";
      }
  ...
  async function send(
    ...
    try {
      /*
       * Re-check availability immediately before submission.
       * The database still performs the final atomic check.
       */
      if (
        !editToken &&
        !editingExisting
      ...
      if (!res.ok) {
        if (res.reason) {
          ...
          setBlocked(
            "Artist already submitted. This artist has already been submitted for another country in this edition.",
          );
        } else if (
          res.error ===
          "duplicate"
          ...
          setBlocked(
            "A confirmation for this country already exists in this round.",
          );
        } else if (
          res.error ===
          "invalid_token"
          ...
          setBlocked(
            "This edit link is no longer valid. Please ask an organiser for a new one.",
          );
        } else {
          setBlocked(
  ...
  if (
    ...
    return (
      <div className="surface animate-rise p-8 text-center">
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          {data.country} will not
          participate in this
          edition. You can close
          this page now.
        </p>
      </div>
    );
    ...
    <div className="animate-rise space-y-6">
      ...
      !editingExisting ? (
        <p className="rounded-lg border border-warning/40 bg-warning/10 px-4 py-3 text-sm">
          ...
          {
            alreadyResponded.country
          }{" "}
          on{" "}
          {new Date(
            alreadyResponded.submitted_at,
          ).toLocaleDateString()}
          . Submitting again will
          be rejected unless an
          organiser reopens it or
          sends you a personal edit
          link.
        </p>
      ) : null}
      ...
      {editingExisting ? (
        <p className="rounded-lg border border-accent/40 bg-accent/10 px-4 py-3 text-sm">
          ...
          for {data.country}. Your
          previous answers are
          pre-filled — update them
          and submit again.
        </p>
      ) : null}
      ...
      <div className="surface space-y-6 p-6 sm:p-8">
        {step === 0 ? (
          <>
            ...
            {data.has_country_account ? (
              <Field
                label="Country Instagram account"
                error={
        ...
        {step === 1 ? (
          <>
            <SectionHeading
              title="Participation"
              subtitle="Are you joining the next edition?"
            />
        ...
        {step === 2 ? (
          <>
            <SectionHeading
              ...
              subtitle="How will your entry be chosen?"
            />

            <Field
              label="How will you select your entry?"
              error={
            ...
            >
              <Choice
                ...
                onChange={(v) =>
                  set(
                    "selection_method",
                    v as ConfirmationPayload["selection_method"],
                  )
                }
              />
            </Field>
        ...
        data.selection_method ===
          ...
          <>
            <SectionHeading
              ...
              subtitle="Tell us about your selection show."
            />
            ...
            <Field
              label="When should the winning entry be known?"
              error={
        ...
        {step === 5 ? (
          <>
            <SectionHeading
              ...
              subtitle="Check everything before sending."
            />
...
function describeDate(
  ...
  if (type === "immediately") {
    return "Can be revealed immediately";
  }
